...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      // KEMBALIKAN FITUR SUB-LINK AGAR VPN BISA IMPORT DATA
      if (url.pathname.startsWith("/sub")) {
        ...
        if (apiPath.startsWith("/sub")) {
          ...
          if (env.PROXY_DB) {
             ...
             if (dictStr) {
                const dict = JSON.parse(dictStr);
                for (const [keyName, ipPort] of Object.entries(dict)) {
                   const cc = keyName.replace(/\d+/g, '').toUpperCase();
                   prxList.push({
              ...
              for (const protocol of filterVPN) {
                ...
                if (protocol == "ss") {
                  ...
                  uri.searchParams.set(
                    "plugin",
                    `${atob(v2)}-plugin${port == 80 ? "" : ";tls"};mux=0;mode=websocket;path=/${prx.keyName};host=${APP_DOMAIN}`
                  );
                } else {
                  uri.username = uuid;
                }
                ...
                // MENGGUNAKAN /sg1 /id1 SEBAGAI PATH DINAMIS
                uri.searchParams.set("path", `/${prx.keyName}`);

                uri.hash = `${result.length + 1} ${getFlagEmoji(prx.country)} ${prx.keyName.toUpperCase()} WS ${
                  port == 443 ? "TLS" : "NTLS"
                } [${serviceName}]`;
  ...
  async function retry() {
    try {
      ...
      tcpSocket.closed.catch((error) => { 
        // ignore close error to prevent crash
      }).finally(() => { 
