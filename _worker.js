...
// Fungsi Sync: Diperbarui untuk menyimpan semua proxy dalam 1 JSON (ALL_ACTIVE_PROXIES)
    ...
    try {
      ...
      if (upgradeHeader === "websocket") {
        ...
        if (ipPortMatch && !/^[A-Z]+\d+$/.test(reqPath)) {
          ...
          if (env.PROXY_DB) {
            // Parsing dari JSON ALL_ACTIVE_PROXIES
            const proxiesRaw = await env.PROXY_DB.get("ALL_ACTIVE_PROXIES", "json");
            const lowerPath = reqPath.toLowerCase(); // id1, sg1
            prxIP = (proxiesRaw && proxiesRaw[lowerPath]) ? proxiesRaw[lowerPath] : "1.1.1.1-443";
          } else {
            prxIP = "1.1.1.1-443";
          }
        } 
          ...
          if (env.PROXY_DB) {
            const proxiesRaw = await env.PROXY_DB.get("ALL_ACTIVE_PROXIES", "json");
            if (proxiesRaw && Object.keys(proxiesRaw).length > 0) {
              const keys = Object.keys(proxiesRaw);
              const randomKey = keys[Math.floor(Math.random() * keys.length)];
              prxIP = proxiesRaw[randomKey];
            } else {
              prxIP = "1.1.1.1-443";
            }
          } else {
