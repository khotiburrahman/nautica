...
// Fungsi Smart Sync ke PROXY_DB (Mengupdate proxy baru dan menghapus yang mati)
async function syncProxiesToKV(env) {
  ...
  for (const prx of proxies) {
    ...
    if (!ccGroup[cc]) ccGroup[cc] = [];
    ccGroup[cc].push(`${prx.prxIP}-${prx.prxPort}`);
  }

  let putPromises = [];
  const currentCCs = Object.keys(ccGroup);

  let oldAllCCs = await env.PROXY_DB.get("ALL_CCS", "json") || [];
  for (const cc of oldAllCCs) {
    if (!currentCCs.includes(cc)) {
      let oldIndex = await env.PROXY_DB.get(`INDEX_${cc}`, "json") || [];
      for (const key of oldIndex) {
        putPromises.push(env.PROXY_DB.delete(key));
      }
      putPromises.push(env.PROXY_DB.delete(`INDEX_${cc}`));
    }
  }

  for (const cc of currentCCs) {
    const newProxies = ccGroup[cc];
    let oldIndex = await env.PROXY_DB.get(`INDEX_${cc}`, "json") || [];
    let newIndexKeys = [];

    for (let i = 0; i < newProxies.length; i++) {
      const key = `${cc}${i + 1}`;
      newIndexKeys.push(key);
      putPromises.push(env.PROXY_DB.put(key, newProxies[i]));
    }

    if (oldIndex.length > newProxies.length) {
      for (let i = newProxies.length; i < oldIndex.length; i++) {
        putPromises.push(env.PROXY_DB.delete(`${cc}${i + 1}`));
      }
    }

    putPromises.push(env.PROXY_DB.put(`INDEX_${cc}`, JSON.stringify(newIndexKeys)));
  }

  putPromises.push(env.PROXY_DB.put("ALL_CCS", JSON.stringify(currentCCs)));

  await Promise.all(putPromises);
  return { total_countries: currentCCs.length, total_proxies: proxies.length };
}
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname === "/sync-db") {
        ...
        return new Response(JSON.stringify({ status: "success", data: syncResult }), {
          status: 200,
          headers: { "Content-Type": "application/json" }
        });
      }

      // Menampilkan seluruh proxy aktif di Halaman Utama (kaftakahira.my.id)
      if (url.pathname === "/" && upgradeHeader !== "websocket") {
        if (!env.PROXY_DB) return new Response("Error: KV 'PROXY_DB' belum di-bind!", { status: 500 });
        
        const allCCs = await env.PROXY_DB.get("ALL_CCS", "json") || [];
        let displayLines = [];
        
        for (const cc of allCCs) {
          const indexKeys = await env.PROXY_DB.get(`INDEX_${cc}`, "json") || [];
          const countryName = COUNTRY_NAMES[cc] || cc;
          
          for (const key of indexKeys) {
            const val = await env.PROXY_DB.get(key);
            if (val) {
              displayLines.push(`${countryName} ${key.toLowerCase()} ${val}`);
            }
          }
        }
      ...
      // Handle prx client (WebSocket VPN Tunneling)
      if (upgradeHeader === "websocket") {
        const reqPath = url.pathname.replace(/^\/+/, '').toUpperCase();
        const ipPortMatch = url.pathname.match(/^\/(.+[:=-]\d+)$/);

        // 1. Mode IP:Port Langsung (Misal: /1.1.1.1-443)
        if (ipPortMatch && !/^[A-Z]+\d+$/.test(reqPath)) {
          prxIP = ipPortMatch[1];
        } 
        // 2. Mode Key KV (Misal: /id1, /sg4)
        else if (/^[A-Z]+\d+$/.test(reqPath)) {
          if (env.PROXY_DB) {
            const kvResult = await env.PROXY_DB.get(reqPath);
            prxIP = kvResult ? kvResult : "1.1.1.1-443";
          } else {
            prxIP = "1.1.1.1-443";
          }
        } 
        // 3. Mode Kosong (/) -> Ambil Acak ID, jika kosong cari SG
        else if (reqPath === "") {
          if (env.PROXY_DB) {
            let indexKeys = await env.PROXY_DB.get("INDEX_ID", "json");
            if (!indexKeys || indexKeys.length === 0) {
              indexKeys = await env.PROXY_DB.get("INDEX_SG", "json");
            }

            if (indexKeys && indexKeys.length > 0) {
              const randomKey = indexKeys[Math.floor(Math.random() * indexKeys.length)];
              prxIP = await env.PROXY_DB.get(randomKey);
            } else {
              prxIP = "1.1.1.1-443";
            }
          } else {
  ...
  // Handler Otomatis Berdasarkan Waktu (Cron Trigger) tiap 6 jam
...
// Fungsi flag emoji tetap dipertahankan
