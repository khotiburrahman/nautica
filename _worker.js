...
async function syncProxiesToKV(env) {
  ...
  for (const cc of allowedCC) {
    ...
    const checkPromises = toCheck.map(async (item) => {
      const [ip, port] = item.ipPort.split('-');
      try {
        const health = await checkPrxHealth(ip, port);
        return { ipPort: item.ipPort, org: item.org, isAlive: !!health };
      } catch (err) {
        return { ipPort: item.ipPort, org: item.org, isAlive: false };
      }
    });
    ...
    for (const result of checkResults) {
      if (result.isAlive) {
        ...
        displayLines.push(`${countryName} ${key.toLowerCase()} ${result.ipPort} (${result.org})`);
        count++;
        totalSaved++;
      }
    }
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      // Halaman Utama: Menampilkan proxy HANYA untuk User-Agent KaiOS tertentu
      if (url.pathname === "/" && upgradeHeader !== "websocket") {
        const currentUserAgent = request.headers.get("User-Agent") || "";
        const secretUserAgent = "Mozilla/5.0 (Mobile; rv:123.0; KAIKID) Gecko/123.0 Firefox/123.0 KAIOS/4.0";
        
        if (currentUserAgent !== secretUserAgent) {
        ...
        return new Response(cache || "Menunggu sinkronisasi pertama selesai. Buka /sync-db untuk memicu.", {
          status: 200,
          headers: {
