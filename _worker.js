...
let serviceName = "";
let APP_DOMAIN = "";
// VARIABEL GLOBAL prxIP DIHAPUS UNTUK MENCEGAH KEBOCORAN KONEKSI
let cachedPrxList = [];
...
async function isProxyAlive(ip, port) {
  try {
    ...
    // PERBAIKAN: Jika error karena dilarang connect ke IP Cloudflare, otomatis loloskan
    if (msg.includes("cloudflare") || msg.includes("local")) {
      return true;
    }
    return false;
  }
...
async function syncProxiesToKV(env) {
  ...
  for (const cc of priorityCC) {
    ...
    let count = 1;
    const toCheck = proxyList.slice(0, 15); // Ambil 15 teratas
    ...
    for (const result of checkResults) {
      if (result.isAlive) {
        const keyName = `${cc.toLowerCase()}${count}`; 
  ...
  return { status: "success", total_active_saved: totalSaved, db_updated: true };
}
        ...
        if (ipPortMatch && !/^[a-z]+\d+$/.test(reqPath)) {
          ...
          if (env.PROXY_DB) {
            ...
            if (dictStr) {
              ...
              localPrxIP = dict[reqPath] ? dict[reqPath] : "1.1.1.1-443";
            }
          }
        } 
        
        return await websocketHandler(request, localPrxIP);
      }

      if (url.pathname.startsWith("/sub") || url.pathname.startsWith("/check") || url.pathname.startsWith("/api/v1")) {
         return new Response("Not implemented in this optimized worker version.", { status: 404, headers: CORS_HEADER_OPTIONS });
      }
...
// ==========================================
// KODE INTI WEBSOCKET & PROTOKOL VPN
// ==========================================
// PERBAIKAN: Menerima localPrxIP sebagai parameter
async function websocketHandler(request, localPrxIP) {
  ...
  readableWebSocketStream.pipeTo(
    new WritableStream({
      async write(chunk, controller) {
        ...
        // PERBAIKAN: Teruskan localPrxIP
        handleTCPOutBound(remoteSocketWrapper, protocolHeader.addressRemote, protocolHeader.portRemote, protocolHeader.rawClientData, webSocket, responseHeader, log, localPrxIP);
      },
    ...
    .pipeTo(
      new WritableStream({
        async write(chunk) {
          ...
          // PERBAIKAN: Jangan tulis data jika WebSocket sudah ditutup (mencegah stream error)
