...
// ----- WebSocket Handler dengan buffer akumulasi -----
function makeReadableWebSocketStream(webSocketServer, earlyDataHeader, log) {
  ...
  return new ReadableStream({
    start(controller) {
      ...
      // Early data (jika ada) tetap dikirim sebagai chunk pertama, tapi tidak wajib
...
async function websocketHandler(request, env, proxyIP, proxyPort) {
  ...
  // ----- BUFFER AKUMULASI UNTUK DETEKSI PROTOKOL -----
    ...
    .pipeTo(
      new WritableStream({
        async write(chunk, controller) {
          // Jika sudah ada koneksi TCP terbuka, kirim data langsung
          ...
          // Jika header sudah diparsis, kirim sisa data ke socket (jika ada)
          if (headerParsed) {
            // Kirim semua data yang tersisa ke socket backend
            if (remoteSocketWrapper.value) {
              const writer = remoteSocketWrapper.value.writable.getWriter();
          ...
          // Coba deteksi protokol jika buffer cukup (minimal 62 byte untuk Trojan, 18 untuk VMess/Neko, 42 untuk SS)
          // Kita tunggu minimal 62 byte agar semua protokol bisa terdeteksi
          if (buffer.length < 62) {
            // Belum cukup, tunggu chunk berikutnya
            return;
          }
          ...
          // Tandai header sudah diparsis
          headerParsed = true;

          // Sisa data setelah header
          ...
          // TCP: buat koneksi ke backend
          handleTCPOutBound(
            remoteSocketWrapper,
            protocolHeader.addressRemote,
            protocolHeader.portRemote,
            protocolHeader.rawClientData, // Gunakan rawClientData dari parseResult
            webSocket,
            responseHeader,
            log,
            proxyIP,
          ...
          // Kirim sisa data (jika ada) setelah koneksi terbuka
          if (remaining.length > 0 && remoteSocketWrapper.value) {
            const writer = remoteSocketWrapper.value.writable.getWriter();
