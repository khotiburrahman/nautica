...
// Fungsi Sync Cepat & Bersih (Hanya SG, ID, MY + Cek Status + Cache Halaman)
async function syncProxiesToKV(env) {
  ...
  // Jalankan penghapusan terlebih dahulu agar KV bersih
  await Promise.all(putPromises);
  putPromises = []; 

  let displayLines = [];
  const finalIndexes = { SG: [], ID: [], MY: [] };
  let totalSaved = 0;

  // 2. Filter, Cek Status, dan Susun Data Baru
  for (const cc of allowedCC) {
    const countryName = COUNTRY_NAMES[cc];
    // Batasi pengecekan ke 12 proxy pertama agar tidak melampaui limit request Worker (Timeout CF)
    const toCheck = ccGroup[cc].slice(0, 12);
    
    // Cek status IP secara paralel agar cepat
    const checkPromises = toCheck.map(async (ipPort) => {
      const [ip, port] = ipPort.split('-');
      try {
        const health = await checkPrxHealth(ip, port);
        return { ipPort, isAlive: !!health };
      } catch (err) {
        return { ipPort, isAlive: false };
      }
    });
    ...
    for (const result of checkResults) {
      if (result.isAlive) {
        const key = `${cc}${count}`;
        finalIndexes[cc].push(key);
        putPromises.push(env.PROXY_DB.put(key, result.ipPort));
        displayLines.push(`${countryName} ${key.toLowerCase()} ${result.ipPort}`);
        count++;
        totalSaved++;
      }
    }
    
    // Simpan index negara untuk fallback random (saat path kosong)
    if (finalIndexes[cc].length > 0) {
      putPromises.push(env.PROXY_DB.put(`INDEX_${cc}`, JSON.stringify(finalIndexes[cc])));
    }
  }

  // 3. Simpan seluruh teks tampilan ke dalam 1 cache agar web loading instan
  ...
  return { 
    ...
    details: {
      SG: finalIndexes.SG.length,
      ID: finalIndexes.ID.length,
      MY: finalIndexes.MY.length
    }
  };
}
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      // Endpoint manual sinkronisasi database
      ...
      // Halaman Utama: Memuat instan 1 kunci cache
      ...
      if (upgradeHeader === "websocket") {
        ...
        if (ipPortMatch && !/^[A-Z]+\d+$/.test(reqPath)) {
          ...
          if (env.PROXY_DB) {
            ...
            if (!indexKeys || indexKeys.length === 0) {
              indexKeys = await env.PROXY_DB.get("INDEX_MY", "json");
            }
