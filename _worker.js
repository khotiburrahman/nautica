...
async function websocketHandler(request) {
  ...
  // LOGGING TAMBAHAN UNTUK TROUBLESHOOT HANDSHAKE CLASH VS V2RAYNG
  console.log("WS Upgrade headers:", {
    upgrade: request.headers.get("Upgrade"),
    host: request.headers.get("Host"),
    "sec-websocket-protocol": earlyDataHeader,
...
// FUNGSI UTAMA YANG DIPERBAIKI UNTUK NORMALISASI FORMAT PAYLOAD WS
function makeReadableWebSocketStream(webSocketServer, earlyDataHeader, log) {
  ...
  return new ReadableStream({
    start(controller) {
        ...
        try {
          if (event.data instanceof ArrayBuffer) {
            ...
            try {
              ...
              console.log("Unknown WS message type", typeof event.data, event.data);
            }
          }
        } catch (err) {
      ...
      if (error) {
        console.log("Early data error:", error);
      } else if (earlyData) {
